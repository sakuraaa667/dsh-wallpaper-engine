...
function pickMediaFile(dir, declared, type) {
  ...
  // Image-capable types (scene / web / image): use the highest-resolution
  // image in the directory — preview files are often small (~800px) while
  // the actual artwork can be 4K+, so a background drawn from the preview
  // looks blurry. Falling back to the largest file by size keeps the
  // wallpaper's real art whenever it ships one.
  return largestFileMatching(dir, (name) => IMAGE_EXTS.has(extname(name).toLowerCase()));
}

function largestFileMatching(dir, predicate) {
  let entries;
  try {
    entries = readdirSync(dir, { withFileTypes: true });
  } catch {
...
function inspectWallpaper(dir, id, source) {
  ...
  const hasImage = file !== null && IMAGE_EXTS.has(extname(file).toLowerCase());
  // Scene-type wallpapers carry their real artwork inside a package file
  // (scene.pkg) — in that case the standalone `file` above is only the small
  // preview. Mark the package so the pkgimage route can lazily extract the
  // embedded high-resolution texture. Skipped when a real standalone image
  // (different from the preview) or a video already wins.
  const pkg = file !== null && (hasVideo || file !== preview)
    ? null
    : firstFileMatching(dir, (name) => extname(name).toLowerCase() === '.pkg');

  return { id, title, type, source, preview, file, hasVideo, hasImage, pkg };
}
